...
class KafkaStreamsJoinStreamStreamApplicationTests {
    private static final String SCHEMA_REGISTRY_SCOPE = KafkaStreamsJoinStreamStreamApplicationTests.class.getName();
    ...
    void setUp() {
        ...
        rekeyLeftOutputTopic = testDriver.createOutputTopic("streams-join-stream-stream-test-" + PERSON_JOIN_STREAM_STREAM_REKEY_TOPIC + "-left-repartition", new StringDeserializer(),
                personSerde.deserializer());
        rekeyRightOutputTopic = testDriver.createOutputTopic("streams-join-stream-stream-test-" + PERSON_JOIN_STREAM_STREAM_REKEY_TOPIC + "-right-repartition", new StringDeserializer(),
                personSerde.deserializer());
        joinOutputTopic = testDriver.createOutputTopic(PERSON_JOIN_STREAM_STREAM_TOPIC, new StringDeserializer(),
    ...
    void shouldJoinWhenTimeWindowIsRespected() {
        ...
        // Explore state store
        WindowStore<String, KafkaPerson> leftStateStore = testDriver.getWindowStore(PERSON_JOIN_STREAM_STREAM_STATE_STORE + "-this-join-store");
        try (KeyValueIterator<Windowed<String>, KafkaPerson> iterator = leftStateStore.all()) {
            ...
            // Time window size is "before" + "after"
        ...
        WindowStore<String, KafkaPerson> rightStateStore = testDriver.getWindowStore(PERSON_JOIN_STREAM_STREAM_STATE_STORE + "-other-join-store");
    ...
    void shouldNotJoinWhenTimeWindowIsNotRespected() {
        ...
        // Upper bound of sliding windows is inclusive, so it needs to be set to 01:05:01 to not match the 5-minutes window of the first record
        rightInputTopic.pipeInput(new TestRecord<>("2", personRight, Instant.parse("2000-01-01T01:05:01.00Z")));
        ...
        try (KeyValueIterator<Windowed<String>, KafkaPerson> iterator = leftStateStore.all()) {
            ...
            assertThat(windowAllen01h00_01h10.key.window().startTime()).isEqualTo("2000-01-01T01:00:00.00Z");
            assertThat(windowAllen01h00_01h10.key.window().endTime()).isEqualTo("2000-01-01T01:10:00.00Z");
            assertThat(windowAllen01h00_01h10.value).isEqualTo(personLeft);
        ...
        WindowStore<String, KafkaPerson> rightStateStore = testDriver.getWindowStore(PERSON_JOIN_STREAM_STREAM_STATE_STORE + "-other-join-store");
        try (KeyValueIterator<Windowed<String>, KafkaPerson> iterator = rightStateStore.all()) {
            KeyValue<Windowed<String>, KafkaPerson> windowAllen01h06_01h16 = iterator.next();
            assertThat(windowAllen01h06_01h16.key.key()).isEqualTo("Allen");
            assertThat(windowAllen01h06_01h16.key.window().startTime()).isEqualTo("2000-01-01T01:05:01.00Z");
            assertThat(windowAllen01h06_01h16.key.window().endTime()).isEqualTo("2000-01-01T01:15:01.00Z");
            assertThat(windowAllen01h06_01h16.value).isEqualTo(personRight);
