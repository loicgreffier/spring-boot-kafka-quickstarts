...
- The usage of the Kafka Streams DSL, including `join()` between KStream and KStream, `selectKey()` and `peek()`.
...
2. Build a new `KafkaJoinPersons` object that holds both persons.
3. Write the resulting `KafkaJoinPersons` objects to a new topic named `PERSON_JOIN_STREAM_STREAM_TOPIC`.
