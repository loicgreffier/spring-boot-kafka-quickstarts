...
public class KafkaStreamsTopology {
     ...
     * stream is joined to the other stream by last name with a left join and 5-minute symmetric join windows with
     * 1-minute grace period. The result is written to the USER_LEFT_JOIN_STREAM_STREAM_TOPIC topic.
     *
     * <p>A left join emits an output for each record in the primary stream. If there is no matching record in the
     * secondary stream, a null value is returned at the end of the join window + grace period. A new record to process
     * is required to make the stream time advance and emit the null result.
     *
