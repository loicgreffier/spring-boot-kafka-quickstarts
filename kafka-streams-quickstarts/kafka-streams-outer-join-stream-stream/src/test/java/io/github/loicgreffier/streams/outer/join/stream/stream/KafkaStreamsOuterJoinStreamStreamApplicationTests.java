...
class KafkaStreamsOuterJoinStreamStreamApplicationTests {
    private static final String SCHEMA_REGISTRY_SCOPE = KafkaStreamsOuterJoinStreamStreamApplicationTests.class.getName();
    ...
    void setUp() {
        ...
        rekeyLeftOutputTopic = testDriver.createOutputTopic("streams-outer-join-stream-stream-test-" + PERSON_OUTER_JOIN_STREAM_STREAM_REKEY_TOPIC + "-left-repartition", new StringDeserializer(),
                personSerde.deserializer());
        rekeyRightOutputTopic = testDriver.createOutputTopic("streams-outer-join-stream-stream-test-" + PERSON_OUTER_JOIN_STREAM_STREAM_REKEY_TOPIC + "-right-repartition", new StringDeserializer(),
                personSerde.deserializer());
    ...
    void shouldEmitRightPersonWhenTimeWindowIsNotRespected() {
        ...
        rightInputTopic.pipeInput(new TestRecord<>("1", personRight, Instant.parse("2000-01-01T01:00:00.00Z")));
        // The default value [right record;null] is emitted after window end + grace period (6 minutes)
        leftInputTopic.pipeInput(new TestRecord<>("2", personLeft, Instant.parse("2000-01-01T01:06:01.00Z")));
        ...
        try (KeyValueIterator<Windowed<String>, KafkaPerson> iterator = leftStateStore.all()) {
            KeyValue<Windowed<String>, KafkaPerson> storedPersonLeftOne = iterator.next();
            assertThat(storedPersonLeftOne.key.key()).isEqualTo("Allen");
            assertThat(storedPersonLeftOne.key.window().startTime()).isEqualTo("2000-01-01T01:06:01.00Z");
            assertThat(storedPersonLeftOne.key.window().endTime()).isEqualTo("2000-01-01T01:16:01.00Z");
            assertThat(storedPersonLeftOne.value).isEqualTo(personLeft);
        }
        ...
        try (KeyValueIterator<Windowed<String>, KafkaPerson> iterator = rightStateStore.all()) {
            ...
            assertThat(storedPersonRightOne.key.window().endTime()).isEqualTo("2000-01-01T01:10:00.00Z");
            assertThat(storedPersonRightOne.value).isEqualTo(personRight);
        }
    }
