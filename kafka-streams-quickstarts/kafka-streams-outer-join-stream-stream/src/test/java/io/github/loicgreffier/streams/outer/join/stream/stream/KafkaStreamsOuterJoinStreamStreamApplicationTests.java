 ...
 */
class KafkaStreamsOuterJoinStreamStreamApplicationTests {
    private static final String SCHEMA_REGISTRY_SCOPE =
        KafkaStreamsOuterJoinStreamStreamApplicationTests.class.getName();
    ...
    void setUp() {
        ...
        properties.setProperty(StreamsConfig.APPLICATION_ID_CONFIG,
            "streams-outer-join-stream-stream-test");
        ...
        rekeyRightOutputTopic = testDriver.createOutputTopic(
            ...
            personSerde.deserializer());
        joinOutputTopic = testDriver.createOutputTopic(PERSON_OUTER_JOIN_STREAM_STREAM_TOPIC,
    ...
    void shouldJoinWhenTimeWindowIsRespected() {
        ...
        WindowStore<String, KafkaPerson> leftStateStore = testDriver.getWindowStore(
            PERSON_OUTER_JOIN_STREAM_STREAM_STATE_STORE + "-outer-this-join-store");
        ...
        WindowStore<String, KafkaPerson> rightStateStore = testDriver.getWindowStore(
            PERSON_OUTER_JOIN_STREAM_STREAM_STATE_STORE + "-outer-other-join-store");
    ...
    void shouldEmitLeftPersonWhenTimeWindowIsNotRespected() {
        ...
        leftInputTopic.pipeInput(
            new TestRecord<>("1", personLeft, Instant.parse("2000-01-01T01:00:00.00Z")));
        ...
        rightInputTopic.pipeInput(
            new TestRecord<>("2", personRight, Instant.parse("2000-01-01T01:06:01.00Z")));
        ...
        try (KeyValueIterator<Windowed<String>, KafkaPerson> iterator = leftStateStore.all()) {
            ...
            assertThat(leftKeyValue.key.window().endTime()).isEqualTo(
                "2000-01-01T01:10:00.00Z");
            assertThat(leftKeyValue.value).isEqualTo(personLeft);
        ...
        try (KeyValueIterator<Windowed<String>, KafkaPerson> iterator = rightStateStore.all()) {
            ...
            assertThat(rightKeyValue.value).isEqualTo(personRight);
    ...
    void shouldEmitRightPersonWhenTimeWindowIsNotRespected() {
        KafkaPerson personRight = buildKafkaPerson("Michael", "Doe");
        rightInputTopic.pipeInput(
            new TestRecord<>("1", personRight, Instant.parse("2000-01-01T01:00:00.00Z")));
        ...
        KafkaPerson personLeft = buildKafkaPerson("John", "Doe");
        leftInputTopic.pipeInput(
            new TestRecord<>("2", personLeft, Instant.parse("2000-01-01T01:06:01.00Z")));

        List<KeyValue<String, KafkaJoinPersons>> results = joinOutputTopic.readKeyValuesToList();

        assertThat(results.get(0).key).isEqualTo("Doe");
        assertThat(results.get(0).value.getPersonOne()).isNull();
        assertThat(results.get(0).value.getPersonTwo()).isEqualTo(personRight);
    ...
    void shouldHonorGracePeriod() {
        ...
        leftInputTopic.pipeInput(
            new TestRecord<>("1", personLeft1, Instant.parse("2000-01-01T01:00:00.00Z")));

        KafkaPerson personLeft2 = buildKafkaPerson("Jane", "Smith");
        leftInputTopic.pipeInput(
            new TestRecord<>("3", personLeft2, Instant.parse("2000-01-01T01:10:30.00Z")));
        ...
        rightInputTopic.pipeInput(
            new TestRecord<>("2", personRight1, Instant.parse("2000-01-01T01:05:00.00Z")));
        ...
        try (KeyValueIterator<Windowed<String>, KafkaPerson> iterator = leftStateStore.all()) {
            ...
            assertThat(windowDoe01h00To01h10.key.key()).isEqualTo("Doe");
            assertThat(windowDoe01h00To01h10.key.window().startTime()).isEqualTo(
                "2000-01-01T01:00:00.00Z");
            assertThat(windowDoe01h00To01h10.key.window().endTime()).isEqualTo(
                "2000-01-01T01:10:00.00Z");
            assertThat(windowDoe01h00To01h10.value).isEqualTo(personLeft1);

            KeyValue<Windowed<String>, KafkaPerson> windowDoe01h10m30To01h20m30 = iterator.next();
            assertThat(windowDoe01h10m30To01h20m30.key.key()).isEqualTo("Smith");
            assertThat(windowDoe01h10m30To01h20m30.key.window().startTime()).isEqualTo(
                "2000-01-01T01:10:30.00Z");
            assertThat(windowDoe01h10m30To01h20m30.key.window().endTime()).isEqualTo(
                "2000-01-01T01:20:30.00Z");
            assertThat(windowDoe01h10m30To01h20m30.value).isEqualTo(personLeft2);
        ...
        try (KeyValueIterator<Windowed<String>, KafkaPerson> iterator = rightStateStore.all()) {
            ...
            assertThat(rightKeyValue.value).isEqualTo(personRight1);
