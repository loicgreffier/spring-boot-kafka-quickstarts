...
class KafkaStreamsBranchApplicationTests {
    private static final String SCHEMA_REGISTRY_SCOPE = KafkaStreamsBranchApplicationTests.class.getName();
    ...
    void setUp() {
        ...
        outputTopicA = testDriver.createOutputTopic(PERSON_BRANCH_A_TOPIC, new StringDeserializer(), personSerde.deserializer());
        outputTopicB = testDriver.createOutputTopic(PERSON_BRANCH_B_TOPIC, new StringDeserializer(), personSerde.deserializer());
        outputTopicDefault = testDriver.createOutputTopic(PERSON_BRANCH_DEFAULT_TOPIC, new StringDeserializer(), personSerde.deserializer());
    }
    ...
    void shouldBranchToTopicA() {
        inputTopic.pipeInput("1", buildKafkaPerson("Calder", "Allen"));
        ...
        assertThat(results.get(0).value.getLastName()).isEqualTo("ALLEN");
    }

    @Test
    void shouldBranchToTopicB() {
        KafkaPerson person = buildKafkaPerson("Abir", "Barron");
    ...
    void shouldBranchToDefaultTopic() {
        KafkaPerson person = buildKafkaPerson("Mathew", "Jennings");
        inputTopic.pipeInput("1", person);
