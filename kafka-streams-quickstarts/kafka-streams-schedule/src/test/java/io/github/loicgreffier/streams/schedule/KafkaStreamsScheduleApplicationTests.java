...
class KafkaStreamsScheduleApplicationTests {
    private static final String SCHEMA_REGISTRY_SCOPE =
        KafkaStreamsScheduleApplicationTests.class.getName();
    ...
    void setUp() {
        ...
        outputTopic = testDriver.createOutputTopic(PERSON_SCHEDULE_TOPIC, new StringDeserializer(),
    ...
    void shouldCountPersonByNationality() {
        inputTopic.pipeInput(
            new TestRecord<>("1", buildKafkaPerson("Marie", "Dupont", CountryCode.FR),
                Instant.parse("2000-01-01T01:00:00.00Z")));
        inputTopic.pipeInput(
            new TestRecord<>("2", buildKafkaPerson("Jean", "Dupont", CountryCode.FR),
                Instant.parse("2000-01-01T01:01:00.00Z")));
        inputTopic.pipeInput(
            new TestRecord<>("3", buildKafkaPerson("John", "Doe", CountryCode.GB),
                Instant.parse("2000-01-01T01:01:30.00Z")));
        inputTopic.pipeInput(
            new TestRecord<>("4", buildKafkaPerson("Giovanni", "Russo", CountryCode.IT),
                Instant.parse("2000-01-01T01:02:00.00Z")));
        testDriver.advanceWallClockTime(Duration.ofMinutes(2));
        inputTopic.pipeInput(
            new TestRecord<>("5", buildKafkaPerson("Philippe", "Dupont", CountryCode.FR),
        ...
        KeyValueStore<String, ValueAndTimestamp<Long>> stateStore =
            testDriver.getTimestampedKeyValueStore(PERSON_SCHEDULE_STATE_STORE);
