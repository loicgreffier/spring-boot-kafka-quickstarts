...
public class KafkaConsumerCircuitBreakerRunner implements ApplicationRunner {
    private static final Logger LOGGER = LoggerFactory.getLogger(KafkaConsumerCircuitBreakerRunner.class);
            ...
            while (true) {
                try {
                    ...
                    for (ConsumerRecord<String, KafkaPerson> message : messages) {
                        LOGGER.info("Received offset = {}, partition = {}, key = {}, id = {}, first name = {}, last name = {}",
                                message.offset(), message.partition(), message.key(), message.value().getId(), message.value().getFirstName(),
                                message.value().getLastName());
                    }
                    ...
                    LOGGER.info("Error while deserializing message from topic-partition {}-{} at offset {}. Seeking to the next offset {}",
